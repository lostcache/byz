...
\end{tikzpicture}
\captionof{figure}{Round 2: Traitor Commander $\gen{0}$ sends inconsistent messages to lieutenants.}
...
\justifying
In this round each lieutenant takes turns acting as the commander for the round and tells all other lieutenants the message they received from the commander of the previous round (In this case Round 2 and the original commander $\gen{0}$). Following table covers all the iteration of Round 1.

When $\gen{1}$ acts as the commander (is the ``acting commander"),  $\gen{2}, \gen{3}, \gen{4}, \gen{5}, \gen{6}$ are the lieutenants. Similarly,

When $\gen{2}$ acts as the commander (is the ``acting commander"),  $\gen{1}, \gen{3}, \gen{4}, \gen{5}, \gen{6}$ are the lieutenants.
...
\end{tabular}
\captionof{table}{Round 0 gossip: Each cell shows the message the sender tells the receiver that it received from the commander, in this case $\gen{1}$. Yellow diagonal: sender's own message from $\gen{1}$. Grey cells: $\gen{1}$ is the acting commander and does not participate in consensus. Blue = Attack (A), Red = Retreat (R).}
\label{tab:traitor_commander_round_0}

\justifying
In our table visualization each lieutenant gossips (tells the other lieutenant what it received from the acting commander, $\gen{1}$ in this case) by filling out the rows, the reason why row headers are marked as $Senders$ and column headers are marked as $Receivers$. And thus the diagonals will simply contain the value that the lieutenant received from the commander.

Notice how each of the lieutenant in Round 0 being loyal, sends out the exact same message that it received from the acting commander $\gen{1}$
...
The majority that was obtained from the Round 0 during the iteration of Round 1 (In our example, when $\gen{1}$ was acting as the commander) is then used by the lieutenants for the same iteration of Round 1. In the example that we consider the majority that was obained from Round 0  when the commander was $\gen{1}$ is then used by the lieutenants as the truth values for Round 1 when $\gen{1}$ was acting as the commander. We visualize it using Table \ref{tab:traitor_commander_round_2_summary}
...
\begin{tabular}{c|c|c|c|c|c}
    ...
    \hline
\end{tabular}
\captionof{table}{The majority obtained from Round 0, visualized using Table ~\ref{tab:traitor_commander_round_0} is now used as the truth in Round 1, which was visualized using Table ~\ref{fig:traitor_commander_round_2} to get this new Table that represents the knowledge after completion of an iteration of Round 1 when $\gen{1}$ completes actings as the commander.}
...
\captionof{figure}{Round 1 Iter 2: Just like previous example for this iteration, $\gen{2}$ acts as the commander for Round 1 and sends the message that it received from the previous commander, $\gen{0}$, The only difference is that $\gen{2}$ being loyal it sends the same message that it received from $\gen{0}$}
\end{center}

\subsection*{\texttt{\large Round 0 for Second Iteration of Round 1}}
Again, just like previous example for this iteration, the rest of the lieutenants ($\gen{1}, \gen{3}, \gen{4}, \gen{5}\ \&\ \gen{6}$) will gossip about the message that they receive from commander $\gen{2}$. The only difference is that $\gen{1}$ being a traitor, it will send contradicting messages to other lieutenants, lying about what it received from $\gen{2}$ to create confusion. The instance is visualized using Table \ref{fig:traitor_commander_round_1_second_iter}
...
\begin{tabular}{@{}c@{\hspace{1em}}c@{}}
  ...
  \adjustbox{valign=c}{\usebox{\mainTableBox}} % Use the saved (and measured) table, vertically centered
\end{tabular}
\captionof{table}{Round 1 Iter 2, Round 0 gossip: Each cell shows the message the sender tells the receiver that it received from the commander, in this case $\gen{2}$. Yellow diagonal: sender's own message from $\gen{2}$. Grey cells: $\gen{2}$ is the acting commander and does not participate in consensus. Blue = Attack (A), Red = Retreat (R).}
\label{fig:traitor_commander_round_1_second_iter}
\end{center}

Once again exactly like before, the lieutenants use the majority of the messages that they receive (represented by column of the table) and this majority is used by the lieutenants for the iteration if Round 1 when $\gen{2}$ was acting as the commander as visualized in Table \ref{tab:traitor_commander_round_1_iter_2}
...
\begin{tabular}{c|c|c|c|c|c}
    ...
    \hline
\end{tabular}
\label{tab:traitor_commander_round_1_iter_2}
\captionof{table}{The majority obtained from Round 0, visualized using Table ~\ref{tab:traitor_commander_round_1_iter_2} is now used as the truth in Round 1, which was visualized using Table ~\ref{fig:traitor_commander_round_2} to get this new Table that represents the knowledge after completion of an iteration of Round 1 when $\gen{2}$ completes actings as the commander.}
\end{center}
...
\end{tikzpicture}
\captionof{figure}{Round 2: Loyal Commander $\gen{0}$ sends a consistent $Attack$ message to all lieutenants.}
...
\begin{tikzpicture}[scale=1, transform shape]
    ...
    \end{scope}
\end{tikzpicture}
\captionof{figure}{Round 1, Iteration 1: Traitor lieutenant $\gen{1}$, acting as commander, falsely reports receiving a \textit{Retreat} command.}
...
\end{tabular}
\captionof{table}{Round 0 gossip when $\gen{1}$ is commander. All lieutenants report receiving `Retreat', leading to a unanimous (but false) majority.}
...
\begin{tabular}{c|c|c|c|c|c}
    ...
    \hline
\end{tabular}
\captionof{table}{Majority from Round 0 (when $\gen{1}$ is acting commander) is incorporated into the knowledge table for Round 1. The first column (for $\gen{1}$) is the value received directly from the commander; the rest are the majorities from the previous Round 0.}
\label{tab:loyal_commander_round_1_iter_1_summary}
\end{center}

\subsubsection*{\texttt{\large Second Iteration of Round 1: A Loyal General Speaks}}
\justifying
...
\begin{tikzpicture}[scale=1, transform shape]
    ...
    \end{scope}
\end{tikzpicture}
\captionof{figure}{Round 1, Iteration 2: Loyal lieutenant $\gen{2}$, acting as commander, honestly reports receiving an \textit{Attack} command.}
\label{fig:loyal_commander_loyal_round_1}
\end{center}

\subsubsection*{\texttt{\large Round 0 for Second Iteration of Round 1}}
\justifying
In the corresponding Round 0, the lieutenants of $\gen{2}$ gossip about the message they received. This time, the traitors $\gen{1}$ and $\gen{4}$ lie, claiming they received \textit{Retreat} from $\gen{2}$, while loyal lieutenants report truthfully. Table~\ref{tab:loyal_commander_round_0_iter_2} shows the messages exchanged. Each loyal lieutenant (columns for $\gen{3}, \gen{5}, \gen{6}$) receives two false \textit{Retreat} messages (from $\gen{1}$ and $\gen{4}$) but three honest \textit{Attack} messages (from $\gen{2}$ via the diagonal, and from $\gen{3}, \gen{5}, \gen{6}$). The majority for all loyal lieutenants is therefore \textit{Attack}.
...
\begin{tabular}{@{}c@{\hspace{1em}}c@{}}
  ...
  \adjustbox{valign=c}{\usebox{\mainTableBox}}
\end{tabular}
\captionof{table}{Round 0 gossip when $\gen{2}$ is commander. Traitors $\gen{1}$ and $\gen{4}$ send false reports, but loyal lieutenants form a majority for \textit{Attack}.}
...
\begin{tabular}{c|c|c|c|c|c}
    ...
    \hline
\end{tabular}
\captionof{table}{Majority from Round 0 (when $\gen{2}$ is acting commander) is incorporated into the knowledge table for Round 1. The second column (for $\gen{2}$) is the value received directly from the commander; the rest are the majorities from the previous Round 0.}
...
\end{tabular}
\captionof{table}{Final knowledge table for all lieutenants after OM(2) completes, consistent with the earlier Round 1 knowledge tables. Each cell shows the majority value from the corresponding Round 0, and the diagonal shows the direct message from the commander.}
