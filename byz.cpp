    ...
    const std::vector<Message>    &decisions)
{
  ...
  {
    if (isActingCommander(actingCommanders, j))
      continue;
    ...
    const std::unordered_set<u64>           &actingCommanders)
{
  ...
  for (u64 i = 0; i < nGenerals; i++)
  {
    if (isActingCommander(actingCommanders, i))
      continue;
  ...
  for (u64 i = 0; i < nGenerals; i++)
  {
    ...
    else if (retreatMessages[i] > attackMessages[i] || attackMessages[i] == retreatMessages[i])
...
std::vector<Message> getMessagesBasedOnRole(
    const u64                     &nGenerals,
    const Role                    &currCommanderRole,
    const Message                 &initialMessage,
    const std::unordered_set<u64> &actingCommanders,
    const u64                     &newCommander)
{
    ...
    const std::vector<Message> &messagesFromPrevCommander)
{
  if (nRounds == 0)
  {
    ...
    exchangeMessages(
        actingCommanders, roles, thisRoundMessages, messagesFromPrevCommander);

    auto decisions =
        getMajorityDecisions(nGenerals, thisRoundMessages, actingCommanders);
  ...
  for (u64 newCommanderId = 0; newCommanderId < nGenerals; newCommanderId++)
  {
    ...
    std::vector<Message> newCommanderMessages = getMessagesBasedOnRole(
        nGenerals,
        roles[newCommanderId],
        messageReceivedFromPrevCommander,
        actingCommanders,
        newCommanderId);
...
void byz(const u64 &nGenerals, const u64 &nTraitors)
{
  u64                     commanderId;
  std::unordered_set<u64> actingCommanders;
  ...
  const Message        originalIntent    = getRandomMessage();
  std::vector<Message> commanderMessages = getMessagesBasedOnRole(
      nGenerals,
      roles[commanderId],
      originalIntent,
      actingCommanders,
  ...
  if (roles[commanderId] == Role::LOYAL)
    assert(finalDecision == originalIntent);
